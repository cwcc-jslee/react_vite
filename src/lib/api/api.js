...
import { createSfaQuery } from '../sfaQueryUtils';
import { processSfaData } from '../sfaDataProcessor';
...
export const updateData = (path, auth_data) =>
  axios.put(`${API_URL}/${path}`, ...auth_data);
...
// 24.08.15 Forecast 관련 추가
export const fetchSalesForecastData = async (year, month, limit = 50) => {
  try {
    const start = 0;
    let allData = [];
    ...
    while (hasMoreData) {
      const query = createSfaQuery(year, month, currentStart, limit);
      //console.log(`Sending request to: ${API_URL}/api/sfa-moreinfos?${query}`);

      const response = await axios.get(`${API_URL}/api/sfa-moreinfos?${query}`);

      // console.log(
      //   `Raw API response for page starting at ${currentStart}:`,
      ...
      hasMoreData = currentStart + limit < totalItems;
      currentStart += limit;
    }

    const completeData = { data: allData };

    // 새로운 데이터 처리 함수 호출
    const processedData = processSfaData(completeData);

    //console.log(
    //   `Processed data for ${year}-${month}:`,
    ...
    console.error(
      `Error fetching sales forecast data for ${year}-${month}:`,
      error,
    );
    if (error.response) {
