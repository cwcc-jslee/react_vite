...
// src/shared/components/ui/button/ActionMenu.jsx
 ...
 *
 * @param {Object} props
 * @param {ControlMenuItem[]} props.baseMenus - 컨트롤 메뉴 항목들 (view, edit)
 * @param {FunctionMenuItem[]} props.subMenus - 기능 메뉴 항목들 (기본정보수정 등)
 * @param {string} [props.className] - 추가 스타일 클래스
 */
const DrawerActionMenu = ({ baseMenus, subMenus, className = '' }) => {
  return (
    <div className={`inline-flex items-center gap-2 ${className}`}>
        ...
        {baseMenus.map((item, index) => (
          <Button
            ...
            className={`
              ...
              ${
                index === baseMenus.length - 1
                  ? 'rounded-r-md rounded-l-none'
                  : ''
              }
              ${
                index !== 0 && index !== baseMenus.length - 1
                  ? 'rounded-none'
                  : ''
              }
              ${item.active ? 'z-10' : ''}
      ...
      {/* 구분선 */}
      {subMenus.length > 0 && <div className="h-6 w-px bg-gray-300 mx-2" />}
...
export default DrawerActionMenu;
