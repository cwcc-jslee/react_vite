...
const FunnelForm = ({ selectedFunnelItems, selectBook }) => {
  // const initialItems = ['경남경총', '벤처기업', '장유중', '기타'];
  console.log('>>(selectbook)>>', selectBook.co_funnel);
  const initialItems = selectBook.co_funnel
    ? selectBook.co_funnel.map((item) => item.attributes.name)
    : [];
  ...
  useEffect(() => {
    ...
    selectedFunnelItems(selectedItems);
  }, [selectedItems]);
